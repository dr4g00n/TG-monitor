...
/// OpenAI API 服务实现
/// 兼容 OpenAI API 标准，可用于 OpenAI、DeepSeek 等
...
impl AIService for OpenAIService {
    async fn analyze(&self, message: &str) -> Result<AnalysisResult, AIError> {
        ...
        debug!("使用 OpenAI 兼容 API 分析消息: {}", preview);
        ...
        debug!("发送请求到 OpenAI API: {}", self.config.base_url);
                ...
                .await
            {
                Ok(response) => {
                    ...
                    if status.is_success() {
                        match response.json::<Value>().await {
                            Ok(result) => {
                                ...
                                let content = result["choices"][0]["message"]["content"]
                                    .as_str()
                                    .ok_or_else(|| {
                                        AIError::ParseError("响应中没有 content 字段".to_string())
                                    })?;
                        ...
                        error!("OpenAI API 返回错误 {}: {}", status, error_text);
                        last_error = Some(AIError::ApiError(format!(
                            "HTTP {}: {}", status, error_text)));
                    }
                }
                Err(e) => {
                    error!("连接 OpenAI API 失败: {}", e);
